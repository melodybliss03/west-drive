...
export default function DevisDialog({ children, defaultType = "particulier" }: DevisDialogProps) {
  ...
  const [form, setForm] = useState({
    nom: "", email: "", telephone: "", nombreVehicules: "1",
    nomEntreprise: "", siret: "",
  ...
  const validate = () => {
    ...
    if (!form.nombreVehicules || parseInt(form.nombreVehicules) < 1) errs.nombreVehicules = "Minimum 1 véhicule.";
    if (type === "entreprise") {
  ...
  const reset = () => {
    setSuccess(false);
    setForm({ nom: "", email: "", telephone: "", nombreVehicules: "1", nomEntreprise: "", siret: "", ville: "", dateDebut: "", heureDebut: "", dateFin: "", heureFin: "", typeVehicule: "" });
    setErrors({});
  };

  const handleOpenChange = (v: boolean) => {
    setOpen(v);
    if (!v) { reset(); setType(defaultType); }
  };
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        {success ? (
          <div className="text-center py-8">
            ...
            <DialogTitle className="font-display text-2xl font-bold mb-2">Demande envoyée !</DialogTitle>
          ...
          <>
            <DialogHeader>
              <DialogTitle className="font-display text-xl font-bold">Demander un devis</DialogTitle>
            </DialogHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-xs font-medium">Type de véhicule <span className="text-primary">*</span></label>
                    <select value={form.typeVehicule} onChange={e => set("typeVehicule", e.target.value)} className={inputClass("typeVehicule")}>
                      <option value="">Sélectionner</option>
                      {vehicleTypes.map(t => <option key={t} value={t}>{t}</option>)}
                    </select>
                    {errors.typeVehicule && <p className="text-xs text-destructive">{errors.typeVehicule}</p>}
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-medium">Nombre de véhicules <span className="text-primary">*</span></label>
                    <Input type="number" min="1" value={form.nombreVehicules} onChange={e => set("nombreVehicules", e.target.value)} className={errors.nombreVehicules ? "border-destructive" : ""} />
                    {errors.nombreVehicules && <p className="text-xs text-destructive">{errors.nombreVehicules}</p>}
                  </div>
                </div>
              </div>
